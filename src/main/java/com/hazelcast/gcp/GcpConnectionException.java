...
public class GcpConnectionException extends RuntimeException {
    ...
    /**
     * Parses the GCP Exception message returned from Google, augmenting it with how to resolve the exception
     *
     * @return parsed exception message with details to user on how to resolve the exception
     */
    @Override
    public String getMessage() {
        if (GcpConnectionException.S_GCP_ERROR_INSUFFICIENT_PERMISSION_SCOPE.equals(getGcpMessage())) {
            return String.format("Your service account does not have permissions to access %s. "
                    + "Please ensure the API access scope for Compute Engine is at least read-only.", getUrl());
        } else if (getGcpMessage() != null
                && getGcpMessage().startsWith(GcpConnectionException.S_GCP_ERROR_COMPUTE_INSTANCES_LIST)) {
            return "Your service account does not have permissions to access \"compute.instances.list\"."
